...
export function currentLocationReducer(state: CurrentLocation = currentLocation, action: Action): CurrentLocation {
    switch (action.type) {
        case actionTypes.FETCH_CURRENT_LOCATION_WEATHER:
            return { ...state, isFetchingCurrentLocation: true, hasCurrentLocationWeather: false };
        case actionTypes.UPDATE_CURRENT_LOCATION_WEATHER:
            return {
                ...state,
                currentLocationData: action.currentLocationData,
                weather: action.weather,
                hasCurrentLocationWeather: true,
                isFetchingCurrentLocation: false
            };
        case actionTypes.FETCH_CURRENT_LOCATION_WEATHER_FAILED:
            return { ...state, isFetchingCurrentLocation: false, hasCurrentLocationWeather: false };
        default:
            return state;
    }
}
